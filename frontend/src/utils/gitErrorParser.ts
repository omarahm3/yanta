...
export function parseGitError(error: unknown): ParsedGitError {
  ...
  if (errorStr.includes("MERGE_CONFLICT")) {
    return {
      ...
      message:
        "Local and remote changes conflict. You need to resolve them manually.",
      technicalDetails: errorStr,
      suggestions: [
  ...
  if (errorStr.includes("PUSH_REJECTED") || errorStr.includes("non-fast-forward")) {
    return {
      ...
      message:
        "Remote has changes you don't have locally. Pull first, then push.",
      technicalDetails: errorStr,
      suggestions: [
        "Run 'Git Pull' to fetch and merge remote changes",
  ...
  if (
    ...
    return {
      ...
      message:
        "Local and remote repositories have completely different histories.",
      technicalDetails: errorStr,
      suggestions: [
  ...
  if (
    errorStr.includes("Already up to date") ||
    errorStr.includes("Already up-to-date")
  ) {
    return {
      type: "INFO",
