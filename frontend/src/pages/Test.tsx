...
const useBlockNoteTestEditor = () => {
  ...
  const upload = React.useCallback(async (file: File) => {
    ...
    setEvents((prev) =>
      [
        `uploadFile(${file.type || "unknown"}, ${file.size} bytes)`,
        ...prev,
      ].slice(0, 10),
    );
    return url;
  ...
  React.useEffect(() => {
    ...
    const acceptList =
      blockSpec?.implementation?.meta?.fileBlockAccept?.join(", ") ?? "";
    ...
    const unsubscribe = editor.onChange(() => {
      ...
      try {
        ...
        setSerialized(
          `Failed to serialise blocks: ${String(serializationError)}`,
        );
      }
    });
    return unsubscribe;
...
export const Test: React.FC<TestProps> = () => {
  ...
  const [canResolvePaths] = React.useState<boolean>(() =>
    CanResolveFilePaths(),
  );
        ...
        <div className="mx-auto flex w-full max-w-3xl flex-col gap-6">
          <header className="space-y-2">
            <h1 className="text-xl font-semibold text-text-bright">
              Wayland File & Clipboard Test
            </h1>
            <p className="text-sm text-text-dim">
              This experimental page helps compare raw HTML pickers and
              clipboard handling against BlockNote&apos;s built-in UI. Use it to
              reproduce platform quirks without the rest of YANTA&apos;s editor
              layer.
            </p>
          </header>
          ...
          <section className="space-y-3 rounded-md border border-border bg-bg px-4 py-3">
            ...
            <p className="text-xs text-text-dim">
              Uses a raw{" "}
              <code className="rounded bg-bg-dark px-1">
                input[type="file"]
              </code>{" "}
              element with{" "}
          ...
          <section className="space-y-3 rounded-md border border-border bg-bg px-4 py-3">
            ...
            <p className="text-xs text-text-dim">
              Focus the area below and press{" "}
              <kbd className="rounded bg-bg-dark px-1">Ctrl</kbd>/
              <kbd className="rounded bg-bg-dark px-1">Cmd</kbd>
              <span>+</span>
              <kbd className="rounded bg-bg-dark px-1">V</kbd> with an image in
              your clipboard.
            </p>
            <div
              tabIndex={0}
          ...
          <section className="space-y-3 rounded-md border border-border bg-bg px-4 py-3">
            ...
            <div
              className={`rounded border px-4 py-2 text-sm ${error
            ...
            <dl className="grid grid-cols-1 gap-2 text-xs text-text">
              <div className="flex items-center justify-between rounded bg-bg-dark px-3 py-2">
                ...
                <dd>
                  {clipboardDebug.types.length
                    ? clipboardDebug.types.join(", ")
                    : "—"}
                </dd>
              </div>
              <div className="flex items-center justify-between rounded bg-bg-dark px-3 py-2">
          ...
          <section className="space-y-3 rounded-md border border-border bg-bg px-4 py-3">
            <h2 className="text-base font-medium text-text-bright">
              BlockNote (baseline configuration)
            </h2>
            <p className="text-xs text-text-dim">
              This embedded editor uses BlockNote without any YANTA-specific
              overrides. Pasted files are converted to object URLs via a local
              upload handler.
            </p>
            <div
          ...
          <div className="flex items-center gap-3 text-xs text-text-dim">
            ...
            <span>
              ...
              <strong>{canResolvePaths ? "available" : "unavailable"}</strong>
            </span>
          </div>
        </div>
      </div>
